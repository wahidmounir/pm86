...
export default (__this) => {
  ...
  let __public_key = __this.bucket.public_key;
  ...
  socket.on(channel, function(data) {
    ...
    data.forEach(function(host, index) {
      ...
      if(__this.hostList.data.hasOwnProperty(server_name)) {
          __this.hostList.data[server_name].update(host);
      } else {
          __this.hostList.data[server_name] = new HostData(host);
      }
      // Remove HostData While Processes Empty
      if(host.data.status.data.process.length === 0) {
          delete __this.hostList[server_name];
          return;
      }
    });
    setTimeout(ask, 1000);
